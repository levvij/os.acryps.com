...
async function NoGap() {
	...
	const reconnect = async () => {
		if (!socket || socket.readyState > 1) {
			...
			const address = configuration.nogap.address
				.replace("%p", location.protocol.replace("http", "ws"))
				.replace("%h", location.host);
			
			return new Promise(done => {
				process.log.action("reconnect", address);
				...
				socket.onclose = () => {
					...
					process.log.info("closed", "connection '" + address + "' closed. Trying to reconnect in " + reconnectAttempts + "s");
